...
//****************************字符串****************************/
    ...
    //加密字符串*
    //encryptStr(字符串,字符格式, 替换方式,替换的字符（默认*）)
    encryptStr: function (obj) {
        ...
        if (_default.regArr.length === 3 && _default.type === 0) {
            regtext = '(\\w{' + _default.regArr[0] + '})\\w{' + _default.regArr[1] + '}(\\w{' + _default.regArr[2] + '})'
            Reg = new RegExp(regtext);
            var replaceCount = this.repeatStr(_default.replacement, _default.regArr[1]);
            return _default.str.replace(Reg, '$1' + replaceCount + '$2')
        }
        //replaceStr('asdasdasdaa',[3,5,3],1)
        //result：***asdas***
        else if (_default.regArr.length === 3 && _default.type === 1) {
            regtext = '\\w{' + _default.regArr[0] + '}(\\w{' + _default.regArr[1] + '})\\w{' + _default.regArr[2] + '}'
            Reg = new RegExp(regtext);
            var replaceCount1 = this.repeatStr(_default.replacement, _default.regArr[0]);
            var replaceCount2 = this.repeatStr(_default.replacement, _default.regArr[2]);
            return _default.str.replace(Reg, replaceCount1 + '$1' + replaceCount2)
        }
        //replaceStr('1asd88465asdwqe3',[5],0)
        //result：*****8465asdwqe3
        else if (_default.regArr.length === 1 && _default.type === 0) {
            regtext = '(^\\w{' + _default.regArr[0] + '})';
            Reg = new RegExp(regtext);
            var replaceCount = this.repeatStr(_default.replacement, _default.regArr[0]);
            return _default.str.replace(Reg, replaceCount)
        }
        //replaceStr('1asd88465asdwqe3',[5],1,'+')
        //result："1asd88465as+++++"
        else if (_default.regArr.length === 1 && _default.type === 1) {
            regtext = '(\\w{' + _default.regArr[0] + '}$)';
            Reg = new RegExp(regtext);
            var replaceCount = this.repeatStr(_default.replacement, _default.regArr[0]);
            return _default.str.replace(Reg, replaceCount)
